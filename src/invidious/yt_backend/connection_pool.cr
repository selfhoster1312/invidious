  ...
  def client(region = nil, &block)
    if region
      ...
      begin
        ...
        conn = HTTP::Client.new(url, tls: make_tls_context())
...
def make_tls_context()
  ...
  elsif ENV["SSL_CERT_DIR"]? != nil
    tls_context.ca_certificates_path = ENV["SSL_CERT_DIR"]
  elsif CONFIG.openssl_ca_certs_file != nil
    tls_context.ca_certificates = CONFIG.openssl_ca_certs_file.not_nil!
  elsif ENV["SSL_CERT_FILE"]? != nil
    tls_context.ca_certificates = ENV["SSL_CERT_FILE"]
  end
